...
(function() {
   ...
   *
   * @description
   * Application-wide API which provides a communication bus between components,
   * navigating the view, generating unique ids, and defining the log levels.
   * Most ngBolt components will inject the Bolt API and it is recommended that
   * all controllers, factories, and components for specific applications also
   * inject the Bolt API.
   *
  ...
  function BltApi($window, config, $location) {
     ...
     *
     * @summary Subscribes to a topic and registers a callback function to be
     * executed when a message is received.
     *
     * @description
     * This is useful when needing to set up lines of communication between
     * controllers and directives, or when needing to complete some function
     * upon completion of an asynchronous function. Subscriptions should be
     * made in the activate method of a controller's function.
     *
     * <div class="note-info">
     * Subscription callback methods are not watched by Angular, therefore any
     * changes to the model must be wrapped in a $timeout so that they are
     * digested by Angular and the view is updated.
     * </div>
     *
     * @param {string} topic The name of the subscription. You can subscribe
     * more than one callback to the same name.
     *
     * @param {function} callback The function for handling the published
     * message.
     *
     * @example
     ...
     *    * @name My Controller
     *    * @module bltDocs
     *    * @description An example controller showing how to subscribe to the Bolt API for messages
     *    * @param {BltApi} api Bolt API
     *    * @param timeout Angular $timeout
     *    * @constructor activate()
     *    *&#47;
     *   function MyController(api, $timeout) {
     *     // Bind To Controller
     *     var ctrl = this;
     *
     *     // Activate Controller On Init
     *     ctrl.$onInit = activate();
     *
     *     // Subscribe To The Bolt API For Messages
     *     function activate() {
     *       // Subscribe To 'test' Messages
     *       api.subscribe('test', function(message) {
     *         // NOTE: This Function Is Called When Any Bolt API Message Is Received
     *
     *         // Check The Message
     *         // NOTE: This Message Could Be Any Type That The Publisher Sent
     *         if ( message == 'new') {
     *           // Do Some Logic
     *           // NOTE: We Wrap The Logic In A $timeout To Trigger Angular's Digest Cycle
     *           $timeout(function() {
     *             ctrl.currentItem = {};    // Not Implemented In This Example
     *             ctrl.createNewItem();     // Not Implemented In This Example
     *           });
     *         }
     *       }  // End Of api.subscribe
     *     }  // End Of activate()
     *   }  // End Of MyController
     *
    ...
    function subscribe(topic, callback) {
      ...
      // Attach The Callback To The Subscription
      subscriptions[topic].push(callback);

      // Log Success
      factory.debug('Subscribed: ', topic);
    }
     ...
     *
     * @summary Removes all callback functions registered to a subscription
     * topic.
     *
     * @description
     * The unsubscribe method removes **all** subscriptions of the provided
     * name. When the scope of a controller or directive is destroyed, it is
     * considered best practice to unsubscribe any subscriptions made in that
     * controller or directive. They will be re-subscribed when they are added
     * back to the scope.
     *
     * <div class="note-warning">
     * The unsubscribe method will delete **all** subscriptions registered to a
     * particular name regardless if that subscription was made from the
     * controller or directive being destroyed. To prevent the unwanted
     * deletion of subscriptions made to the same name but from a different
     * controller/directive, use unique names on subscriptions made from
     * different controllers or directives.
     * </div>
     *
     * @param {string} topic The name of the
     * {@link BltApi#subscribe subscription} you wish to delete.
     *
     * @param {function} [callback] An optional callback function to run after
     ...
     *    * @name My Controller
     *    * @module bltDocs
     *    * @description An example controller showing how to unsubscribe from messages from the Bolt API
     *    * @param {BltApi} api Bolt API
     *    * @param timeout Angular $timeout
     ...
     *       // Subscribe To 'test' Messages
     *       api.subscribe('test', function(message) {
     *         // NOTE: This Function Is Called When Any Bolt Message Is Received
     *
     *         // Check The Message
     *         // NOTE: This Message Could Be Any Type That The Publisher Sent
     *         if ( message == 'new') {
     ...
     *       }  // End Of api.subscribe
     *
     *       // Call Destroy Function When Controller Is Destroyed
     *       $scope.$on('$destroy', destroy);
     *     }  // End Of activate()
     *
     *     // Clean Up The Controller When Destroyed
     *     function destroy() {
     *       // Unsubscribe From Subscriptions
     *       api.unsubscribe('test', function() {
     *         // NOTE: This Function Is Called When The Unsubscribe Was Successful
     *
     *         // Log Success
     *         api.log("Successfully Removed All Subscriptions For 'test' Topic!");
     *       });
     *
     *       // Delete Controller From Scope
     *       // NOTE: This Is Only Needed If You Use controllerAs In The HTML Template
     *       delete $scope.MyCtrl;
     *     } // End Of destroy()
     *   }  // End Of MyController
     *
    ...
    function unsubscribe(topic, callback) {
      ...
      // Log Success
      factory.debug('Unsubscribed: ' + topic);
     ...
     * @name BltApi#publish
     *
     * @summary Publishes a message to the Bolt API, which is then received by
     * subscribers to the same topic.
     *
     * @description The publish method provides a built-in ability to interact
     * with ngBolt components from within a controller or custom directive as
     * well as send custom messages between ngBolt application modules.
     *
     * @param {String} topic The name of the subscription you are publishing to.
     * If opening a modal or panel, this will be the ID of the modal or panel,
     * otherwise it will be the name you passed to
     * {@link BltApi#subscribe BltApi.subscribe}.
     *
     * @param {String|Object} message A string or object to be passed to the
     * callback of the subscription you are publishing to. If opening a modal
     * or panel, `message` will be 'open'. Likewise, if closing a panel or
     * modal, it will be 'close'. If publishing to a custom subscription, this
     * will be the string or object you expect to handle.
     *
     * @example
     ...
     *    * @name My Controller
     *    * @module bltDocs
     *    * @description An example controller showing how to publish a message to the Bolt API
     *    * @param {BltApi} api Bolt API
     *    * @param timeout Angular $timeout
     *    * @constructor activate()
     *    *&#47;
     ...
     *     // Subscribe To The Bolt API For Messages
     *     function activate() {
     *       // Open A Modal With An ID Of 'logout'
     *       api.publish('logout', 'open');
     *
     *       // Close A Panel With An ID Of 'editUser'
     *       api.publish('editUser', 'close');
     *
     *       // Publish A Custom Message To A Custom Subscriber
     *       var message = {
     *         action: 'edit',
     *         data: {
     *           name: 'Foo'
    ...
    function publish(topic, message) {
      ...
      subscriptions[topic].forEach(function(callback) {
        // Send Message To Subscriber (By Calling The Callback Function)
        callback(message);
      });

      // Log Success
      factory.debug('Published: ' + topic, message);
    }
     ...
     *
     * @summary A helper function for updating Angular's $location.path()
     * and/or $location.searchParams().
     *
     * @description
     * Updates $location with the provided path and search params. If only
     * `path` is provided,
     * [$location.path()](https://docs.angularjs.org/api/ng/service/$location)
     * will be called, if `searchParams` are included,
     * [$location.url()](https://docs.angularjs.org/api/ng/service/$location)
     * is called. If `searchParams` is an emtpy object, any search parameters
     * on the current url will be removed.
     *
     * <div class="note-info">
     * If the current URL has search parameters but only `path` is passed as a
     * parameter, only the path will change and the search parameters will
     * still be applied to the URL. This can be useful when switching views when
     * filtering by some value(s).
     * </div>
     *
     * <div class="note-tip">
     * See {@link views Routes & Views} for more information on defining views.
     * </div>
     *
     * @param {string} path The path of the view you want to open.
     *
     * @param {object} [searchParams] Search parameters to append to the
     * supplied path. If the object is empty, search parameters will be removed.
     *
     * @example
     * <example name="views">
     *   <javascript>
     ...
     *    * @name My Controller
     *    * @module bltDocs
     *    * @description An example controller showing how to switch views
     *    * @param {BltApi} api Bolt API
     *    * @constructor activate()
     *    *&#47;
     ...
     *     ctrl.$onInit = activate();
     *
     *     // Switch View Examples
     *     function activate() {
     *       // NOTE: The Following Example Assume The Base URL Is http://example.com/
     *
     *       // Change The URL To:
     *       //   HTML5: http://example.com/some/path
     *       //   Non-HTML5: http://example.com/#!/some/path
     *       api.switchViews('/some/path');
     *
     *       // Change The URL To:
     *       //   HTML5: http://example.com/some/path?foo=value1&bar=value2
     *       //   Non-HTML5: http://example.com/#!/some/path?foo=value1&bar=value2
     *       api.switchViews('/some/path', {foo: 'value1': bar: 'value2'});
     *
     *       // If The Current URL Is: http://example.com/some/path?foo=value1&bar=value2
     *       // Change The URL To:
     *       //   HTML5: http://example.com/some/other/path
     *       //   Non-HTML5: http://example.com/#!/some/other/path
     *       api.switchViews('some/other/path', {});
     *
     *       // If The Current URL Is: http://example.com/some/path?foo=value1&bar=value2
     *       // Change The URL To:
     *       //   HTML5: http://example.com/some/other/path?foo=value1&bar=value2
     *       //   Non-HTML5: http://example.com/#!/some/other/path?foo=value1&bar=value2
     *       api.switchViews('some/other/path');
     *
     *       // If The Current URL Is: http://example.com/some/path?foo=value1&bar=value2
     *       // Change The URL To:
    ...
    function switchViews(path, searchParams) {
      // TODO: Add Ability To Replace URL
      // TODO: Check This Logic

      // If The User Supplied A Path
      if(path === null) {
        // If They Sent An Empty Search Params Object
        if(searchParams && Object.keys(searchParams).length == 0) {
          // Reset Search Parameters And Redirect To Current Path
          $location.url($location.path());
        } else if(searchParams) {
          // NOTE: They Sent One Or More Search Params

          // Update The Search Params
          $location.search = searchParams
        }
      } else {
        // NOTE: The User Defined A Path
        ...
        if(searchParams && Object.keys(searchParams).length == 0) {
          try {
            ...
            // Log Error
            factory.error("There was error trying to switch views. See https://ngbolt.github.io/#!/api/blt_core/service/BltApi#switchViews on how to switch views.", error);
          }
        } else if(searchParams) {
          // NOTE: They Sent One Or More Search Params

          try {
            // Update Search Parameters & Change Path
          ...
          // NOTE: They Just Want To Update The Path
     ...
     *
     * @summary A helper function to get the current view.
     *
     * @description
     * Returns the url information of the current view. Returns the absolute
     * url if no getter is set.
     *
     * @param {string} [getter] Getter method to call from Angular's
     * [$location](https://docs.angularjs.org/api/ng/service/$location) service.
     *
     ...
     * @name BltApi#uuid
     *
     * @summary Generates and returns a new UUID.
     *
     * @description Generates and returns a
     * [Version 4 universally unique identifier](https://en.wikipedia.org/wiki/Universally_unique_identifier)
     * conforming to [RFC 4122](https://tools.ietf.org/html/rfc4122).
     *
     ...
     *
     * @description
     * Sets the application-wide log level, affecting the logs that are printed
     * to the console when invoking any of the following BltApi logging
     * functions: `trace`, `debug`, `info`, `warn` and `error`. The levels are
     * listed in order of precedence from lowest to highest. When the log level
     * is set, the log levels that are enabled include that specific level and
     * any level of higher precedence. If you set the level to 'info', the
     * levels enabled will be `info`, `warn` and `debug`.
     *
     * <div class="note-info">
     * The log level that is assigned will remain in affect until the user
     * clears it via the {@link BltApi#clearLogLevel} method. This will persist
     * across browser and user sessions.
     * </div>
     *
     * @param {number} level The log level to assign.
     *
     * @returns {boolean} Whether or not the given log level was successfully
     * applied.
     */
    function setLogLevel(level) {
      // Try To Call The Assign Log Level
      if(assignLogLevel(level)) {
        // Set The Log Level To Local Storage
     ...
     *
     * @description Clears the saved log level and re-assigns the default log
     * level. This can be set with the defaultLogLevel in your
     * {@link blt_config Configuration} file.
     */
    function clearLogLevel() {
      // Remove The Saved Log Level From Local Storage
...
(function () {
     ...
     */
    angular.module('bltDocs', [
        'app.core',     // Core Factory (Shared Data)
        'app.main',     // Main Application Logic

        'app.home',    // Docs Home Page
                        // TODO: About Section
                        // TODO: Docs Section
                        // TODO: API Section
                        // TODO: App Section
    ...
    function config($locationProvider, $routeProvider, API_DATA, APP_API_DATA, ROUTES) {
        ...
        // NOTE: --------------------------------------------
        // NOTE: Stuff Below Is Fixed
        // NOTE: --------------------------------------------
        // Set HTML5 Mode
        $locationProvider.html5Mode(false);
        
        // Loop Through Dynamic Routes
        angular.forEach(ROUTES, function (parent) {
            // Add The Route To The Route Provider
            $routeProvider.when(normalizePath(parent.path), {
                controller: parent.controller,
                controllerAs: parent.controllerAs,
                templateUrl: parent.template
            });
        });
        
        // TODO: Loop Through API Routes (Both Docs & App)

        // Default To Home Page If No Route Is Found
        $routeProvider.otherwise('/');
    }

    config.$inject = ["$locationProvider", "$routeProvider", "API_DATA",  "APP_API_DATA", "ROUTES"];
...
(function() {
   ...
   * @name bltDocs.docs
   * 
   * @description The module for the docs section of the
   * ngBoltJS project website
   * 
   */
  angular.module('docs', []);
...
(function() {
   ...
   */
  function ApiController(sharedFactory, API_DATA, $window, DOCS) {
    var ctrl = this;
    ...
    function activate() { 
      ...
      sharedFactory.current.page = 'api';
      ...
      ctrl.search = sharedFactory.search;

      angular.forEach(DOCS, function(doc) {

        // Guard Against Bad Data
        if(doc.module && ['home', 'docs', 'about', 'api'].indexOf(doc.module) == -1) {

          // If It's A Module
          if(doc.type == 'module') {
            // If It Doesn't Exist
            if(!ctrl.modules[doc.name]) {
              // Add It
              ctrl.modules[doc.name] = {
                type: doc.type,
                name: doc.name.split('blt_')[1],
              ...
              // NOTE: Exists; Was Added By Non-Module

              // Get The Docs
              var docs = ctrl.modules[doc.name].docs;
            ...
            // NOTE: It's Not A Module

            // If The Module Doesn't Exist
            if(!ctrl.modules[doc.module]) {
              // Create A Small Wrapper
              ctrl.modules[doc.module] = {
                docs: []
              }
            }

            // Add The Doc
            ctrl.modules[doc.module].docs.push({
              type: doc.type,
              name: doc.name,
              template: doc.template,
              deprecated: doc.deprecated,
  ...
  ApiController.$inject = ['sharedFactory', 'API_DATA', '$window', 'DOCS'];
})();
(function() {
  'use strict';
...
(function () {
    ...
    /**
     * Holds all the shared information for the application.
     *
     * @returns {Object} sharedFactory
     */
    function sharedFactory() {
        ...
        // Public Data
        factory.current = {
            page: 'home',   // Current Page: Default To The Home Page
            path: '',       // Current URL Path: Set By Watcher In Main Controller
            sidebar: {      // Current Open Sidebar Items: Set By User Selection
                tutorial: false,
                fundamentals: false,
                best: false,
                resources: false,
...
(function() {
   ...
   * @module bltDocs.docs
   * 
   * @description The controller for the docs
   * section of the ngBoltJS project website
   */
  function DocsController(sharedFactory, $window) {
    var ctrl = this;
    ...
    function activate() { 
      // Shared.sidebarData = API_DATA;
      // Shared.showSidebar = true;
      sharedFactory.current.page = 'docs';
    }
  ...
  DocsController.$inject = ['sharedFactory', '$window'];
})();
(function () {
    'use strict';
    ...
    /**
     * Controls data needed by the ngBolt home page. Remembers if console
     * animation already ran and disables it.
     *
     * Routes: [/]
     ...
     * @constructor activate()
     */
    function HomeController(sharedFactory, $timeout) {
        var ctrl = this;
        ...
        /**
         * Initialize & bind to data in the shared factory.
         */
        function activate() {
            // Sets The Current Page Group
            sharedFactory.current.page = 'home';

            // Init "Skip" Variable In Shared Factory If It Doesn't Exist
            if (!sharedFactory.skip) {
                sharedFactory.skip = {};
            }

            // Init "Skip" "Console" In Shared Factory If It Doesn't Exist
            if (!sharedFactory.skip.console) {
                sharedFactory.skip.console = false;
            }

            // Bind To Shared Factory Data
            ctrl.skip = sharedFactory.skip;

            // Set Flag For Skipping The Console Animation (After First Round)
            // NOTE: CSS Animation Takes 4.4 Seconds (_home.scss)
            $timeout(function () {
                sharedFactory.skip.console = true;
            }, 5000);
        }
    }
...
(function () {
    ...
    /**
     * Top level controller for the application. This controller will only be
     * initialized once per visit.
     *
     * @param {BltApi} BltApi ngBolt API
     * @param {sharedFactory} sharedFactory Shared Factory
     * @param {DOCS} DOCS List Of Available Documentation Pages
     * @param {$timeout} $timeout Angular $timeout
     * @param {$scope} $scope Angular $scope
     * @param {$location} $location Angular $location
     * @constructor activate()
     */
    function MainController(BltApi, sharedFactory, DOCS, $timeout, $scope, $location) {
        var ctrl = this;
        ...
        // Public Functions
        ctrl.open              = open;
        ctrl.toggleSidebarItem = toggleSidebarItem;

        /**
         * Initialize & bnd to data in the shared factory.
         */
        function activate() {
            // Init "Skip" Variable In Shared Factory If It Doesn't Exist
            if (!sharedFactory.skip) {
            ...
            // Init "Skip" "Appbar" In Shared Factory If It Doesn't Exist
            if (!sharedFactory.skip.appbar) {
                sharedFactory.skip.appbar = true;
            }

            // Bind To Shared Factory Data
            ctrl.skip    = sharedFactory.skip;
            ctrl.current = sharedFactory.current;

            // Set Flag For Skipping The App Bar Animation (After First Open)
            // NOTE: CSS Animation Takes 0.5 Seconds (_main.scss)
            $timeout(function() {
                sharedFactory.skip.appbar = false;
            }, 500);
        ...
        /**
         * When the path of the url successfully changes, set
         * the path using the path from the $location service
         */
        function onRouteChange(event, route) {
            // Trigger The Digest
            $timeout(function() {
                // Reset Sidebar "Open" Items
                sharedFactory.current.sidebar.tutorial     = false;
                sharedFactory.current.sidebar.fundamentals = false;
                sharedFactory.current.sidebar.best         = false;
                sharedFactory.current.sidebar.resources    = false;
                sharedFactory.current.sidebar.examples     = false;

                // Set The Path
                sharedFactory.current.path = $location.path();

                // Set The Header
                sharedFactory.current.hash = $location.hash();
        ...
        function toggleSidebarItem(item) {
            sharedFactory.current.sidebar[item] = !sharedFactory.current.sidebar[item];
        }
    }
